...
public:
    ...
    double predictOutput_final(vector<Sample> e,int n,double lon) {
        vector<double> h(hiddenDim, 0.0), c(hiddenDim, 0.0);
...
string decodeOutput(double val) {
    if(0.5<=val&&val<1.5)return "Mưa";
    else if(1.5<=val&&val<2.5)return "Nắng";
    else if(2.5<=val&&val<3.5)return "Tuyết";
    else if(3.5<=val&&val<4.5)return "Phùn";
    else if(4.5<=val&&val<5.5)return "Sương mù";
